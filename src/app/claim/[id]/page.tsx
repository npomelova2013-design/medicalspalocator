...
export default async function ClaimPage({ params }: Props) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
      <nav className="text-sm text-[#2C1810]/50 mb-6">
        <Link href="/" className="hover:text-[#B76E79]">Home</Link>
        <span className="mx-2">/</span>
        <Link href={`/med-spa/${spa.url_slug}`} className="hover:text-[#B76E79]">{spa.business_name}</Link>
        <span className="mx-2">/</span>
        <span className="text-[#2C1810]">Claim</span>
      </nav>

      <h1 className="text-2xl md:text-3xl font-bold text-[#2C1810] mb-2">Claim Your Listing</h1>

      <div className="bg-[#FFF8F0] border border-[#B76E79]/20 rounded-lg p-4 mb-6">
        <h3 className="font-semibold text-[#2C1810]">{spa.business_name}</h3>
        <p className="text-sm text-[#2C1810]/60">{spa.city}, {spa.state}</p>
        <StarRating rating={spa.google_rating} count={spa.google_reviews_count} size="sm" />
      </div>

      <div className="bg-white border border-[#B76E79]/20 rounded-xl p-6">
