...
export function TreatmentMarquee() {
  ...
  return (
    <section className="relative bg-[#1a1a2e] py-8 overflow-hidden" aria-label="Treatments">
      <div className="marquee-track whitespace-nowrap">
        {items.map((treatment, i) => (
          <span key={`${treatment}-${i}`} className="inline-flex items-center">
            <span className="font-serif italic text-2xl md:text-4xl text-[#833AB4]/30 font-semibold tracking-editorial px-6 md:px-10">
              {treatment}
            </span>
            <span className="w-2 h-2 rounded-full bg-[#833AB4]/20 flex-shrink-0" />
          </span>
        ))}
      </div>
    </section>
